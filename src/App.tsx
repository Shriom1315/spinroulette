    ...
    <div className="relative min-h-screen flex flex-col bg-[#f1f5f9] text-slate-900 selection:bg-indigo-100 no-scrollbar">
      ...
      <nav className="h-20 bg-white/70 backdrop-blur-xl border-b border-white/50 px-6 md:px-12 flex items-center justify-between shadow-sm z-[100] sticky top-0">
        ...
        <div className="flex items-center gap-3 md:gap-6">
          {/* View Toggles (Admin Only) */}
          {isAdmin && (
            <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200 mr-2">
              <button
                onClick={() => setCurrentView('arena')}
                className={`flex items-center gap-2 px-4 py-1.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${currentView === 'arena' ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400 hover:text-slate-600"
                  }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-1.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${currentView === 'leaderboard' ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400 hover:text-slate-600"
                  }`}
              >
                <BarChart3 size={14} />
                Rankings
                      ...
                      <div className="bg-white p-2 rounded-[3rem] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.5)]">
                        <div className="bg-indigo-600 text-white px-12 md:px-20 py-10 md:py-16 rounded-[2.5rem] border-8 border-indigo-500 flex flex-col items-center gap-6 relative overflow-hidden">
                          ...
                          <div className="flex items-center gap-4 relative z-10">
                            ...
                            <span className="text-xs font-black uppercase tracking-[0.5em] text-indigo-200">Chance to Pitch!</span>
                          </div>
                    ...
                    {isAdmin ? (
                      ...
                      <div className="text-center p-8 glass-card max-w-sm w-full border-indigo-100/50">
                        ...
                        <p className="text-sm font-medium text-slate-500 mt-2 leading-relaxed">
                          Wait for the Organizer to spin the wheel.
                        </p>
                      </div>
                    )}
                  </div>
